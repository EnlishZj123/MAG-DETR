...
class BaseConfig(object):
    ...
    def __init__(self) -> None:
        ...
        # training loop knobs
        # - eval_freq: evaluate every N epochs (0 disables periodic eval; still evals on last epoch if enabled in solver)
        # - save_last_freq: save last.pth every N epochs (0 disables periodic save; still saves on last epoch)
        self.eval_freq: int = 1
        self.save_last_freq: int = 1

        # early stopping (disabled by default)
        # patience counts evaluation events (i.e., every eval_freq epochs).
        self.early_stop_patience: int = 0
        self.early_stop_metric: str = 'coco_eval_bbox'
        self.early_stop_index: int = 0
        self.early_stop_mode: str = 'max'  # 'max' (higher is better) or 'min'
