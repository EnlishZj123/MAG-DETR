...
To benchmark complex crowded scenes, prepare CrowdHuman in COCO format first:

```shell
python tools/prepare_crowdhuman.py --splits val
```

Then run validation with the CrowdHuman config and the COCO2017 checkpoint:
...
This keeps the COCO-80 detector unchanged. Only the CrowdHuman ground truth is
converted to COCO style with a single `person` category, so the reported
metrics are person-only AP / AR on the val split.
